...
class PretrainDataset(Dataset):
    def __init__(self, args, tokenizer, parquet):
        super().__init__()
        self.parquet = parquet
        self.args = args
        self.tokenizer = tokenizer['chartok_coords']
        self.transform = get_transforms(args.input_size, 'pretrain', augment=True, debug=args.debug)
        self.max_len = FORMAT_INFO['chartok_coords']['max_len']
    ...
    def getitem(self, idx):
        ...
        ref['chartok_coords'] = torch.LongTensor(label[:self.max_len])
...
def bms_collate(batch):
    ...
    seq_format_key = next((k for k in formats if k in ['chartok_coords', 'atomtok_coords']), None)
    if seq_format_key:
        MAX_LEN = FORMAT_INFO[seq_format_key]['max_len']
    ...
    for key in seq_formats:
        padded_seqs = pad_sequence(refs[key][0], batch_first=True, padding_value=PAD_ID)
        current_max_len = padded_seqs.size(1)
        if current_max_len < MAX_LEN:
            pad_size = MAX_LEN - current_max_len
            padded_seqs = F.pad(padded_seqs, (0, pad_size), 'constant', PAD_ID)
        elif current_max_len > MAX_LEN:
            padded_seqs = padded_seqs[:, :MAX_LEN]
            for i in range(len(refs[key][1])):
                refs[key][1][i].clamp_max_(MAX_LEN)
        # this padding should work for atomtok_with_coords too, each of which has shape (length, 4)
        refs[key][0] = padded_seqs
