...
def _fix_benzene_typos(mol):
    if mol is None:
        return None
    try:
        mol_rw = Chem.RWMol(mol)
        mol_rw.UpdatePropertyCache(strict=False)
        Chem.GetSymmSSSR(mol_rw)
        
        modified = False
        
        while True:
            ring_info = mol_rw.GetRingInfo()
            found_7_ring = False
            
            for ring in ring_info.AtomRings():
                if len(ring) == 7 and all(mol_rw.GetAtomWithIdx(idx).GetAtomicNum() == 6 for idx in ring):
                    db_count = sum(1 for i in range(7) if mol_rw.GetBondBetweenAtoms(ring[i], ring[(i+1)%7]) is not None \
                        and mol_rw.GetBondBetweenAtoms(ring[i], ring[(i+1)%7]).GetBondType() == Chem.BondType.Double)
                    
                    if db_count >= 3:
                        degree2_idx = -1
                        for idx in ring:
                            if mol_rw.GetAtomWithIdx(idx).GetDegree() == 2:
                                degree2_idx = idx
                                break
                        if degree2_idx != -1:
                            neighbors =[nbr.GetIdx() for nbr in mol_rw.GetAtomWithIdx(degree2_idx).GetNeighbors()]
                            n1, n2 = neighbors[0], neighbors[1]
                            
                            mol_rw.RemoveAtom(degree2_idx)
                            
                            if n1 > degree2_idx: n1 -= 1
                            if n2 > degree2_idx: n2 -= 1
                            
                            mol_rw.AddBond(n1, n2, Chem.BondType.SINGLE)
        ...
        for ring in ring_info.AtomRings():
            if len(ring) == 6 and all(mol_rw.GetAtomWithIdx(idx).GetAtomicNum() == 6 for idx in ring):
                db_count = sum(1 for i in range(6) if mol_rw.GetBondBetweenAtoms(ring[i], ring[(i+1)%6]) is not None 
                               and mol_rw.GetBondBetweenAtoms(ring[i], ring[(i+1)%6]).GetBondType() == Chem.BondType.DOUBLE)
                
                if db_count == 2 or db_count == 3:
                    for idx in ring:
                        mol_rw.GetAtomWithIdx(idx).SetIsAromatic(True)
                    for i in range(6):
                        bond = mol_rw.GetBondBetweenAtoms(ring[i], ring[(i+1)%6])
                        if bond is not None:
...
def _postprocess_smiles(smiles, coords=None, symbols=None, edges=None, molblock=False, debug=False):
    ...
    try:
        ...
        # get gid of noise fragaments
        if '[HH]' in pred_smiles:
            frags = pred_smiles.split('.')
            frags = [f for f in frags if f != '[HH]']
            pred_smiles = '.'.join(frags)
            
        # fix wrong typos
        unbracketed_typos = {
            'S(F)(F)(F)(F)(F)F': 'S(F)(F)(F)(F)F',
            'F[Sb](F)(F)(F)(F)F': 'F[Sb](F)(F)(F)F'
        }
        
        for wrong, correct in unbracketed_typos.items():
            if wrong in pred_smiles:
                pred_smiles = pred_smiles.replace(wrong, correct)
