...
def top_k_logits(logits, k):
    ...
    return torch.where(logits < kth_value, torch.full_like(logits, -torch.inf), logits)


def sample_tokens(logits, temperature=1.0, top_k=0):
    if top_k > 0:
